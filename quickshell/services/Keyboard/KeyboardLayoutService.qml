...
Singleton {
  ...
  onCurrentLayoutChanged: {
    ...
    if (layoutChanged) {
      if (Settings.data.notifications.enableKeyboardLayoutToast) {
        const message = qsTr("toast.keyboard-layout.changed", {
                                  "layout": currentLayout.toUpperCase()
                                });
        ToastService.showNotice(message, "", "", 2000);
      }
      console.log("KeyboardLayout", "Layout changed from", previousLayout, "to", currentLayout);
    }
  ...
  Component.onCompleted: {
    console.log("KeyboardLayout", "Service started");
  ...
  Timer {
    ...
    onTriggered: {
      ...
      previousLayout = currentLayout;
      console.log("KeyboardLayout", "Service initialized, current layout:", currentLayout);
    }
  }
