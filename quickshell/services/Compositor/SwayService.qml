...
Item {
  ...
  function initialize() {
    ...
    try {
      ...
      initialized = true;
      console.log("SwayService", "Service started");
    } catch (e) {
      console.error("SwayService", "Failed to initialize:", e);
    }
  }
  ...
  Process {
    ...
    onExited: function (exitCode) {
      if (exitCode !== 0 || !accumulatedOutput) {
        console.error("SwayService", "Failed to query outputs, exit code:", exitCode);
        accumulatedOutput = "";
        return;
      }
      ...
      try {
        ...
        console.error("SwayService", "Failed to parse outputs:", e);
      } finally {
  ...
  Process {
    ...
    onExited: function (exitCode) {
      if (exitCode !== 0 || !accumulatedOutput) {
        console.error("SwayService", "Failed to query inputs, exit code:", exitCode);
        accumulatedOutput = "";
        return;
      }
      ...
      try {
        ...
        for (const input of inputsData) {
          if (input.type == "keyboard") {
            ...
            console.log("SwayService", "Keyboard layout switched:", layoutName);
            break;
          }
        }
      } catch (e) {
        console.error("SwayService", "Failed to parse inputs:", e);
      } finally {
        // Clear accumulated output for next query
        accumulatedOutput = "";
      }
  ...
  function safeUpdateWorkspaces() {
    try {
      ...
      console.error("SwayService", "Error updating workspaces:", e);
    }
  }
  ...
  function safeUpdateWindows() {
    try {
      ...
      console.error("SwayService", "Error updating windows:", e);
    }
  }
  ...
  function switchToWorkspace(workspace) {
    try {
      ...
      console.error("SwayService", "Failed to switch workspace:", e);
    }
  }
  ...
  function focusWindow(window) {
    try {
      ...
      console.error("SwayService", "Failed to switch window:", e);
    }
  }
  ...
  function closeWindow(window) {
    try {
      ...
      console.error("SwayService", "Failed to close window:", e);
    }
  }
  ...
  function logout() {
    try {
      ...
      console.error("SwayService", "Failed to logout:", e);
    }
  }
}
