...
Item {
  ...
  function initialize() {
    ...
    try {
      ...
      initialized = true;
      console.log("HyprlandService", "Service started");
    } catch (e) {
      console.error("HyprlandService", "Failed to initialize:", e);
    }
  }
  ...
  Process {
    ...
    onExited: function (exitCode) {
      if (exitCode !== 0 || !accumulatedOutput) {
        console.error("HyprlandService", "Failed to query monitors, exit code:", exitCode);
        accumulatedOutput = "";
        return;
      }
      ...
      try {
        ...
        console.error("HyprlandService", "Failed to parse monitors:", e);
      } finally {
  ...
  Process {
    ...
    onExited: function (exitCode) {
      if (exitCode !== 0 || !accumulatedOutput) {
        console.error("HyprlandService", "Failed to query devices, exit code:", exitCode);
        accumulatedOutput = "";
        return;
      }
      ...
      try {
        ...
        for (const keyboard of devicesData.keyboards) {
          if (keyboard.main) {
            ...
            console.log("HyprlandService", "Keyboard layout switched:", layoutName);
          }
        }
      } catch (e) {
        console.error("HyprlandService", "Failed to parse devices:", e);
      } finally {
        // Clear accumulated output for next query
        accumulatedOutput = "";
      }
  ...
  function safeUpdateWorkspaces() {
    try {
      ...
      console.error("HyprlandService", "Error updating workspaces:", e);
    }
  }
  ...
  function safeUpdateWindows() {
    try {
      ...
      console.error("HyprlandService", "Error updating windows:", e);
    }
  }
  ...
  function handleActiveLayoutEvent(ev) {
    try {
      ...
      console.error("HyprlandService", "Error handling activelayout:", e);
    }
  }
  ...
  function switchToWorkspace(workspace) {
    try {
      ...
      console.error("HyprlandService", "Failed to switch workspace:", e);
    }
  }
  ...
  function focusWindow(window) {
    try {
      if (!window || !window.id) {
        console.log("HyprlandService", "Invalid window object for focus");
        return;
      }
      ...
      console.error("HyprlandService", "Failed to switch window:", e);
    }
  }
  ...
  function closeWindow(window) {
    try {
      ...
      console.error("HyprlandService", "Failed to close window:", e);
    }
  }
  ...
  function logout() {
    try {
      ...
      console.error("HyprlandService", "Failed to logout:", e);
    }
  }
}
